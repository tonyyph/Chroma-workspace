...
stability, semantic roles) but they are *legible only to people who already know
what ΔE00 is*, a market of perhaps tens of thousands, most of whom already own
Adobe tools.
...
uncannily right and *then* say why.
...
non-negotiables in `06` and `07`: the atmosphere is *computed*, never invented;
the track is *fetched*, never hallucinated.
...
is chosen over the literal "Bộ nhớ màu" because *ký ức* is lived memory and *bộ
nhớ* is computer memory.
...
- **Working Sets** — the professional container, reachable from a Collection's
  ...
  gap analysis, and export. Presented as a *mode*, not a tab.
...
3. Users return to *replay* memories, not only to make new ones.
...
- Local-first: every memory, photo, palette and track *reference* stays on the
